...
export const OptionsOnProductPage: React.FC<Props> = ({
  selectedProduct,
  product,
}) => {
  const {
  ...
  const isItemsInFavorite
    = itemsInFavorite.some(item => item.phoneId === detailedId);
  ...
  const handleClick
  = (event: React.MouseEvent<HTMLButtonElement>, type: string) => {
    ...
    <div className="options__general-block">
      <div className="options__choose-settings">
        ...
        <div className="options__buttons-box">
          {capacityAvailable.map(param => {
            const isSelectedCapacity = capacity === param;
            const newPathRam = path
              .replace(capacity.toLowerCase(), param.toLowerCase());

            return (
              <Link to={{ pathname: `../${newPathRam}` }} key={param}>
                <button
                  className={classNames(
                    'options__button',
                    {
